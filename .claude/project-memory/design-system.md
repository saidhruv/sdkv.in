...
- **Résumé version axis (résumé page only):** `/resume` has THREE orthogonal axes — **version** × **mode** (Normal|ATS, default ATS) × **theme**. Version is a scalable, registry-driven concept (`RESUME_VERSIONS = [{slug,label,focus,file,download}]`, single source of truth at the top of `resume/index.html`'s head; default `ai`). The selector is an **adaptive edition picker**: one compact trigger (`.edition-trigger`, mono/red, shows the active edition) that opens a **popover on desktop** and a **bottom sheet on mobile** (`.edition-panel` + `.edition-backdrop`, built at `<body>` level so they escape the header's `backdrop-filter` containing block, which would otherwise capture `position:fixed`). The panel lists each edition (label + one-line focus, active marked with a red left-rule) and **also houses the Normal|ATS Format toggle** — collapsing the old four-control cluster to `Back · wordmark · [Edition ▾] · theme · Download`. Registry-driven, keyboard-accessible (menu roles + `aria-checked`, Esc, focus return, arrow nav), reduced-motion aware. (Replaced the earlier native `<select>` "Tailored for" dropdown, which clashed with the chrome and crowded the bar.) Content is authored as per-version `.rv-<slug>` blocks inside the single `.sheet` and single `.doc`; generic CSS shows only the active version (`:root[data-resume-variant="<slug>"] .rv-<slug>`), the rest collapse to zero height (one `fit()` path). `html[data-resume-variant]` is set pre-paint by the head bootstrap (order `?variant=` registry-validated → `localStorage.resumeVariant` → default). Versions today: `ai` (AI & Technology Leader) and `frontend` (Frontend Architect). Each version has its own N×3 PDFs (`<file>-{light,dark,ats}.pdf`) and its own forced download filename.
- **Résumé edition picker — shipped form (2026-09-09):** the trigger lives in a dedicated full-width **red "EDITION —" strip** below the top bar (relocated out of the actions cluster). The menu is **flush, borderless, left-anchored** to the selector (scrollbar-safe; `--ep-left = trigger.left`) and equal width. Opening plays a **sequenced ~250ms animation** — a frosted-glass backdrop fades in (content-only blur on desktop; full-cover haze on mobile), then the menu **rolls down** (desktop `clip-path`) / **springs up** (mobile); dismiss reverses it. Discoverability: **auto-reveal on every visit** (briefly opens, no focus steal, dismiss on interaction), an inline **"Also available: <edition> →"** switch in the poster + ATS masthead, and an **"AI & Frontend editions"** hint by the homepage Résumé pill. Requires `--ease`/`--ease-out` (now defined in the résumé `:root` — they were undefined, which silently disabled transitions).
